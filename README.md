...
Deleting a User has the consequence of barring futher API access with those credentials. This way, a Participant's data can be kept, while locking out their client's access [TODO: CHANGE FROM PROTECT]. A Django model, in `orbit.phaseone.models`
...
5. Create MEDIA_ROOT directory for uploads
6. Set appropriate environment variables. In production, a `.env` file is used. For development, these can be set via shell exports as part of a run server script. See below.
...
6. As root, run provision script  
   ```
...
7. Create first web app user (or run with a copy of the production database)
